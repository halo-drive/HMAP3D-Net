...
def augment_intrinsics(K, augment_range=0.3):
    ...
    This teaches the network to generalize across different cameras
    
    Args:
        K: [B, 3, 3] intrinsics matrix
        augment_range: perturbation range (e.g., 0.3 = ±30%)
    
    Returns:
        K_augmented: perturbed intrinsics
    """
    K_aug = K.clone()
    ...
    for i in range(batch_size):
        ...
        # Slightly perturb principal point (smaller range)
...
def prepare_targets(batch_boxes_3d):
    ...
    if len(all_depth) > 0:
        targets = {
            ...
            'foreground': torch.ones(sum(len(d) for d in all_depth))  # All are foreground
        }
    else:
        targets = {
            'depth': torch.zeros(0),
...
def train_epoch(model, dataloader, optimizer, loss_fn, scaler, device, epoch, augment_intrinsics_flag=True):
    """Train for one epoch with mixed precision and intrinsics augmentation"""
    model.train()
    ...
    for batch in pbar:
        ...
        # Augment intrinsics during training
...
def main():
    ...
    print("\nBuilding model V2 (intrinsics-conditioned + PointPillars heads)...")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        print(f"  Train Loss: {train_losses['total']:.4f} (D:{train_losses['depth']:.3f} "
              f"Dim:{train_losses['dimension']:.3f} Dir:{train_losses['direction']:.3f} "
              f"Rot:{train_losses['rotation']:.3f} IoU:{train_losses['iou']:.3f})")
        print(f"  Val Loss:   {val_losses['total']:.4f} (D:{val_losses['depth']:.3f} "
              f"Dim:{val_losses['dimension']:.3f} Dir:{val_losses['direction']:.3f} "
              f"Rot:{val_losses['rotation']:.3f} IoU:{val_losses['iou']:.3f})")
