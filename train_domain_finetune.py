...
def visualize_samples(model, dataloader, device, output_dir, epoch, num_samples=4):
    ...
    actual_samples = min(num_samples, len(batch['images']))
    
    images = batch['images'][:actual_samples].to(device)
    boxes_2d = batch['boxes_2d'][:actual_samples]
    boxes_3d = batch['boxes_3d'][:actual_samples]
    intrinsics = batch['intrinsics'][:actual_samples].to(device)
    ...
    print(f"  → Saved {actual_samples} visualizations to {viz_dir}")
...
def main():
    ...
    train_dataset = DomainDataset(
        ...
        val_split=0.2  # 2 out of 10 for validation
    )
    ...
    # Configure DataLoader based on number of workers
    dataloader_kwargs = {
        'batch_size': args.batch_size,
        'collate_fn': collate_fn,
        'pin_memory': True if args.workers > 0 else False,
    }
    
    if args.workers > 0:
        dataloader_kwargs['num_workers'] = args.workers
